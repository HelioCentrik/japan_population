...
# Computed eagerly at import — single lightweight query.
# layout.py imports this for the initial embedded pyramid figure.
# Stays module-level so it's available before run() is called.
_prewarm_axis_max = get_pyramid_axis_max(None)


def _valid_years(metric: str) -> list[int]:
    """Census years within the coverage window for a given metric."""
...
def run() -> None:
    """Build or load the figure cache. Called in a background thread by main.py."""
